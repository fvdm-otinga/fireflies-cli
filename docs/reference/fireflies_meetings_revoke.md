## fireflies meetings revoke

Revoke shared meeting access for an email (GraphQL: revokeSharedMeetingAccess) [destructive]

```
fireflies meetings revoke <id> [flags]
```
...
```
      ...
      --email string        Email address to revoke access for (required)
