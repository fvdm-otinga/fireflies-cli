## fireflies config get

Print one profile config value

### Synopsis

Print the value of a config key for the active profile.
Valid keys: api_key, endpoint.
The api_key is masked unless --show-secret is provided.

```
fireflies config get <key> [flags]
```
...
```
  -h, --help          help for get
      --show-secret   Print api_key unmasked
```
